...
"""
    def long_video_vace_fun(self, model_h, model_l, clip, vae, width, height, loopback_crossfade, vace_prompt_list, seed, steps_h, cfg_h, sampler_h, scheduler_h, steps_l, cfg_l, sampler_l, scheduler_l, 
        ...
        for item in vace_prompt_list:
            ...
            seg_model_h = item.get('model_override_h', model_h)
            seg_model_l = item.get('model_override_l', model_l)
            ...
            if nag_parameters is None:
                sample_h = nodes.common_ksampler(seg_model_h, seed, steps, cfg_h, sampler_h, scheduler_h, conditions['positive'], conditions['negative'], conditions['out_latent'], 
                                                    denoise=1.0, disable_noise=False, start_step=0, last_step=steps_h, force_full_denoise=False)[0]
                sample_l = nodes.common_ksampler(seg_model_l, seed, steps, cfg_l, sampler_l, scheduler_l, conditions['positive'], conditions['negative'], sample_h, 
                                                    denoise=1.0, disable_noise=True, start_step=steps_h, last_step=10000, force_full_denoise=True)[0]
            else:
                latent_image = conditions['out_latent']["samples"]
                latent_image = comfy.sample.fix_empty_latent_channels(seg_model_h, latent_image)
                ...
                callback_h = latent_preview.prepare_callback(seg_model_h, steps)
                callback_l = latent_preview.prepare_callback(seg_model_l, steps)
                disable_pbar = not comfy.utils.PROGRESS_BAR_ENABLED
                nag_scale = nag_parameters['nag_scale']
                nag_tau = nag_parameters['nag_tau']
                nag_alpha = nag_parameters['nag_alpha']
                nag_sigma_end = nag_parameters['nag_sigma_end']
                nag_negative = conditions['negative']
                nag_sample_out_h = sample_with_nag(seg_model_h, noise_h, steps, cfg_h, nag_scale, nag_tau, nag_alpha, nag_sigma_end, sampler_h, scheduler_h, conditions['positive'], conditions['negative'], 
                ...
                nag_sample_out_l = sample_with_nag(seg_model_l, noise_l, steps, cfg_l, nag_scale, nag_tau, nag_alpha, nag_sigma_end, sampler_l, scheduler_l, conditions['positive'], conditions['negative'], 
